...
public class CalculateDayOfMonth {
    public static void main(String[] args) {
        ...
        while (true) {
            System.out.println("Bao Phuc - Enter month you wanna know (full, abbreviation, number): ");
            monthInput = scanner.nextLine().trim();
            if (checkValidMonth(monthInput)) {
                break;
            } else {
                System.out.println("Bao Phuc - Invalid month. Enter again! ");
            }
        }

        // Check valid year input
        while (true) {
            System.out.println("Bao Phuc - Enter year you wanna know (xxxx): ");
            yearInput = scanner.nextLine().trim();
            if (yearInput.matches("\\d{4}")) {
                year = Integer.parseInt(yearInput);
                break;
            } else {
                System.out.println("Bao Phuc - Invalid year. Enter again!");
            }
        }

        // Get the number of days in the month
        int dayOfMonth = getDaysOfMonth(monthInput, year);
        System.out.println("Bao Phuc - The number of days in " + monthInput + " in " + year + " is: " + dayOfMonth);
    }
